 ...
 * version 0.9.8) a bug that makes the "ENTER" key impossible to detect when
	...
	private void initialize () {
		addListener(inputListener = new ClickListener() {
			...
			public boolean touchDown (InputEvent event, float x, float y, int pointer, int button) {
				...
				if (stage != null) {
					setFocus();
					stage.setKeyboardFocus(ChatTextField.this);
				}
				keyboard.show(true);
				return true;
			}
			...
			public boolean keyTyped (InputEvent event, char character) {
				...
				if (stage != null && stage.getKeyboardFocus() == ChatTextField.this) {
					...
					// Commented this out because we only have one textfield.
